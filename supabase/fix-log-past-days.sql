-- Only days that have ended can be logged. Adds the allow_same_day switch
-- (off in production) and tightens challenge_log. Safe to rerun; no data changes.
alter table public.challenge_config add column if not exists allow_same_day boolean not null default false;
