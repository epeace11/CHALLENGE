...
-- An unchanged row is not written at all (return null skips it): the hourly rescore rewrites closed weeks, and a write Postgres skips is one Realtime never publishes.
...
-- Push reminders: see supabase/push-reminders.sql and the Reminders section of README.md.
