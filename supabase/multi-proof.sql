-- Several screenshots per answer (September 18, 2026). The proof column keeps
-- one text value: newline-separated storage paths. Every path must be the
-- member's own upload, and an answer carries at most six. Safe to rerun;
-- replaces one function body and changes no data.
