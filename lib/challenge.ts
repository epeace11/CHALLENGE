...
/** Instant (ms) when logging for `date` locks: 2 pm the next day, Toronto time, DST-aware. */
export function lockTime(date:string){const next=shift(date,1);let guess=Date.UTC(+next.slice(0,4),+next.slice(5,7)-1,+next.slice(8,10),14);for(let i=0;i<2;i++){const parts=new Intl.DateTimeFormat('en-CA',{timeZone:'America/Toronto',year:'numeric',month:'2-digit',day:'2-digit',hour:'numeric',hourCycle:'h23'}).formatToParts(new Date(guess));const get=(t:string)=>parts.find(p=>p.type===t)?.value??'';const localDay=`${get('year')}-${get('month')}-${get('day')}`,hour=Number(get('hour'));const dayDiff=(Date.UTC(+localDay.slice(0,4),+localDay.slice(5,7)-1,+localDay.slice(8,10))-Date.UTC(+next.slice(0,4),+next.slice(5,7)-1,+next.slice(8,10)))/864e5;guess-=(dayDiff*24+hour-14)*36e5;}return guess}
