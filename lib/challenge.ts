...
{id:'gym',title:'Go to gym',question:'Did you go to the gym today?',description:'Condo or regular gym. One visit per day. Weeks run Monday to Sunday. Four visits per full week; Sep 15–20: three. Oct 12–14: one.',days:'Weekly',group:'Movement',weekly:true}];
...
export function clampDate(d:string){return d<'2026-09-15'?'2026-09-15':d>'2026-10-14'?'2026-10-14':d}
export function activeRules(person:Person,date:string){const dow=new Date(date+'T12:00Z').getUTCDay();return rules.filter(r=>(!r.person||r.person===person)&&(r.days!=='Sun–Thu'||dow<=4)&&(r.days!=='Fri–Sat'||dow>4))}
...
export function week(d:string){if(d<='2026-09-20')return {start:'2026-09-15',end:'2026-09-20',target:3};if(d<='2026-09-27')return {start:'2026-09-21',end:'2026-09-27',target:4};if(d<='2026-10-04')return {start:'2026-09-28',end:'2026-10-04',target:4};if(d<='2026-10-11')return {start:'2026-10-05',end:'2026-10-11',target:4};return {start:'2026-10-12',end:'2026-10-14',target:1}}
