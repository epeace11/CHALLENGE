...
/** Total gift cost of `points` active points: the n-th point costs $n. */
export function money(points: number) {
...
export const locked = (e: Entry | undefined, now: number) =>
  !!e &&
  closed(e.day, now) &&
  ['confirmed', 'missed', 'excused', 'conceded'].includes(e.status) &&
  e.proposed_done === null;
/** Latest day whose logging window has opened: yesterday, but never before the start or after the end. */
export const maxLoggable = (now: number) => {
  const y = shift(toronto(new Date(now)), -1);
  return y < START ? shift(START, -1) : y > END ? END : y;
};
export const dayNumber = (now: number) =>
  Math.min(
    ...
    Math.max(
      0,
      Math.round(
        (Date.parse(toronto(new Date(now)) + 'T00:00Z') -
          Date.parse(START + 'T00:00Z')) /
          864e5,
      ) + 1,
    ),
  );

export const entryKey = (uid: string, rule: string, day: string) =>
  `${uid}|${rule}|${day}`;
const key = entryKey;
export const index = (data: Data) => {
  const m = new Map<string, Entry>();
  for (const e of data.entries) m.set(key(e.user_id, e.rule_id, e.day), e);
  return m;
};
  ...
  return rules
    ...
    .map((r) => {
      ...
      for (const d of days()) {
        ...
        if (t === 'done' || t === 'review') {
          ...
          for (const n of [7, 14, 30])
            if (run === n && !reached[n]) reached[n] = d;
        } else if (t === 'missed') run = 0;
      }
      ...
      return {
        ...
        best,
        reached,
        dollars: cost.find((c) => c.rule === r.id)?.dollars ?? 0,
      };
    });
}
...
export function habitOrder(stats: Record<string, HabitStat[]>): Rule[] {
  const agg = new Map<
    string,
    { rule: Rule; missed: number; rate: number; n: number }
  >();
  for (const list of Object.values(stats))
    for (const s of list) {
      const a = agg.get(s.rule.id) ?? {
        rule: s.rule,
        missed: 0,
        rate: 0,
        n: 0,
...
export const nextMissCost = (data: Data, uid: string) =>
  pointCosts(data, uid).size + 1;
export function costByHabit(data: Data, uid: string) {
  const costs = pointCosts(data, uid),
    m = new Map<string, { rule: string; dollars: number; count: number }>();
  for (const q of data.points) {
    const c = costs.get(q.id);
    if (!c) continue;
    const a = m.get(q.rule_id) ?? { rule: q.rule_id, dollars: 0, count: 0 };
    a.dollars += c;
    a.count++;
    m.set(q.rule_id, a);
  }
  return [...m.values()].sort(
    (a, b) => b.dollars - a.dollars || b.count - a.count,
  );
}

/** Misses per person on a closed day, daily habits only. */
const missesOn = (data: Data, p: Profile, d: string, now: number, ix: Index) =>
...
export function perfectDays(
  ...
  for (const d of days()) {
    if (!closed(d, now)) break;
    const rs = dailyRules(p.name, d);
    if (
      rs.length &&
      rs.every((r) =>
        ['done', 'excused'].includes(tone(ix.get(key(p.id, r.id, d)), d, now)),
      )
    ) {
      list.push(d);
...
export function badges(
  ...
  const stats = habitStats(data, p, now, ix),
    perfect = perfectDays(data, p, now, ix),
    over = closed(END, now),
    partner = data.profiles.find((o) => o.id !== p.id);
  let cleanWeek: string | undefined;
  {
    let run = 0,
      prev = '';
    for (const d of perfect.list) {
      run = prev === shift(d, -1) ? run + 1 : 1;
      prev = d;
      if (run === 7) {
        cleanWeek = d;
  ...
  return [
    ...
    b(
      'untouchable',
      'Untouchable',
      'One habit with zero misses all challenge',
      over && stats.some((s) => s.missed === 0 && s.done > 0) && END,
    ),
    b(
      'gym_regular',
      'Gym regular',
      'Every weekly gym target hit',
      over && !gymShort && END,
    ),
    b(
      'halfway',
      'Strong half',
      'Fewer than five active misses by day 15',
      closed(half, now) && halfPoints < 5 && half,
    ),
    b(
      'gracious',
      'Gracious',
      'Forgave your partner at least once',
      undefined,
      !!partner &&
