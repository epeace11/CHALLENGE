...
- `lib/` — plain TypeScript, no React.
  ...
  - `types.ts` the database row types and `Data`.
...
- `hooks/` — auth, data loading with polling and Realtime, the clock, new-badge tracking, the WebMCP tool.
...
- `components/<page>/` — one folder per page: `overview/`, `log/`, `review/`, `progress/`, `rules/`; `auth/` for sign-in and password reset; `dialogs/` for the entry, habit and day views and the dispute and forgiveness dialogs; `shared/` for pieces used on several pages (status pill, screenshots, journal, progress bar).
...
- `npm test` runs `tests/database.mjs` (PostgreSQL schema and authorization/scoring tests in an isolated PGlite database with dummy Auth accounts) and `tests/progress.mjs` (the statistics in `lib/progress.ts`).
- `npm run build`
